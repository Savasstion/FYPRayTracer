...
// __host__ __device__ float EvaluateEmitterPMF_TLAS(const Mesh_GPU* meshes, const LightTree* tlas_tree, const LightTree::ShadingPointQuery& sp,
//     uint32_t emitterIndex, uint32_t& randSeed)
// {
//     float out = 0.0f;
//     float randFloat = MathUtils::randomFloat(randSeed);
//
...
//         if (node.isLeaf) {
//             uint32_t blasIndex = node.emitterIndex;   //  index to actual emmisive triangle
//             out = EvaluateEmitterPMF_BLAS(meshes[blasIndex].lightTree_blas, sp, emitterIndex, pmfAcc,randFloat);
//             return out;
//         }
//
//         // Internal node: left child index is in offset, right child index stored in emmiterIndex
...
//
//         // Leaf: sample emitter from leaf
//         if (node.isLeaf && node.emitterIndex == emitterIndex) {
//             out = currentPMF * pmfAcc;
//             return out;
//         }
//
//         // Internal node: left child index is in offset, right child index stored in emitterIndex
