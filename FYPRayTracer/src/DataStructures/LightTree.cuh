...
public:
    ...
    struct Node
    {
        ...
        bool isLeaf = false;
        uint32_t emitterIndex = static_cast<uint32_t>(-1);
        //  if leaf, this stores the index of actual emmisive triangle. If internal node, then this stores the right child index

        Node(uint32_t triIndex, const glm::vec3& barycentricCoord, const AABB& box, const ConeBounds& orient,
             float emittedEnergy)
            : energy(emittedEnergy), numEmitters(1), offset(0), bounds_o(orient), bounds_w(box),
              position(barycentricCoord), isLeaf(true), emitterIndex(triIndex)
        {
        }
    ...
    uint32_t BuildHierarchyRecursively_SAOH(Node* outNodes, uint32_t& outCount, Node* work, uint32_t first,
                                            uint32_t last);
    void ClearLightTree();
...
__host__ __device__ __forceinline__ float ComputeClusterImportance(const LightTree::ShadingPointQuery& shadingPoint,
                                                                   const LightTree::Node& cluster)
{
...
//  Obsolete
__host__ __device__ __forceinline__ LightTree::SampledLight PickLight(const LightTree* tree,
                                                                      const LightTree::ShadingPointQuery& sp,
                                                                      uint32_t& randSeed)
{
